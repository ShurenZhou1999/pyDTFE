...
class DTFE:
    def __init__(self, pos, 
        ...
        Parameters
        ----------
        pos : Ndarray, shape (Nparts, 3 or 2). The position of particles.
        ...
        if self.Ndim not in [2,3]:
            raise ValueError("Input Position & Velocity Array should be shape liked (Nparts, 3 or 2)")
    ...
    def GridVel(self, vel, nmesh=None):
        ...
        Parameters
        ----------
        vel : Ndarray, shape (Nparts, 3 or 2), matched the position shape. The velocity of particles.
    ...
    def SampleVel(self, vel, samplings:np.array ):
        ...
        Parameters
        ----------
        vel : Ndarray, shape (Nparts, 3 or 2). The velocity of particles.
        samplings : int. The position of the interpolaton sampling point, shape as (Nsampling, 3 or 2).
        '''
        if vel.dtype != np.float32:
            vel = vel.astype(np.float32)
